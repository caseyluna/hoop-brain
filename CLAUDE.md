...
As of 2026-08-30: `Team` works end-to-end for both leagues (ingest → GCS → BigQuery raw → dbt staging/mart → Postgres sync → API), league-tagged throughout. `get_players()` exists in `ingestion-engine/src/modules/nba_api.py` but nothing downstream consumes it yet — player entity resolution (CAL-150) hasn't landed, so Postgres/the API are Teams-only. `model-engine` and dbt (beyond the teams mart) are stubs. Scheduled daily ingestion runs via GitHub Actions (`scheduled-ingest.yml`) with failure alerting (a GitHub issue naming the failed step). **Deployed:** Postgres is Neon (`hoop-brain` project, Teams-only cutover); the API is live on Vercel at **https://api-rho-umber-13.vercel.app** (`GET /health`, `GET /api/v1/teams`) — CORS defaults open (`CORS_ORIGINS` env var to restrict). CI/lint/test scaffolding is mature across services. Roadmap: Stage 0 (league foundation) → 1 (Player + entity resolution) → 2 (scheduling + deploy) → 3 (games/box + WNBA) → 4 (pbp/lineups) → 5 (impact models + traits) → 6 (profile API + leaderboards = MVP) → 7 (grades/trends) → 8 (contracts/cap/surplus) → 9 (Team Fit + Trade Machine) → 10+ (expansion). Frontend phases: F0 foundations → F1 core views → F2 tools → F3 expanded.
